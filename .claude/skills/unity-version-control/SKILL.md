...
→ Claude 在改動 `EditorSettings.asset` / `VersionControlSettings.asset` 前必須讀過確認沒動到這兩個欄位。
...
- 根目錄：`.gitignore`、`.gitattributes`、`README.md`、`.claude/`（見最末段「協作配置」）
...
→ Claude 收到「刪除/搬移某個 Asset」的指令時：用 `Bash` 同時處理 `Foo.xxx` 和 `Foo.xxx.meta`，或建議使用者透過 Unity Editor 操作。
...
→ Claude 看到使用者要 commit 一張 >10MB 的圖/模型時：**主動提醒** LFS 還沒設定，並列出需要的步驟。
...
Claude 收到「commit / push / 開 PR」的指令時，先依序檢查：
...
`.gitignore` 規則：`.claude/` 整個資料夾**會進 git**（讓團隊共用 skill 與專案層 Claude 設定），只忽略個人本地設定 `.claude/settings.local.json`。實際規則：

```
/.claude/settings.local.json
```

→ 新加入專案層的 `.claude/skills/<name>/SKILL.md` 都會自動納入版控；只有本機個人偏好（API key、本地 permission）會留在本機。
